...
#ifdef KEEP_LOG
    static std::ofstream out_BallResult(qPrintable(QString("%1/BallistResult.txt").arg(log_path))); //logging result of classification
#endif //KEEP_LOG
...
CtrlAIR_BALLIST::CtrlAIR_BALLIST()
{
//#ifdef KEEP_LOG
//    AIR_BALL::out_info << "Constructor: CtrlAIR_BALLIST   " << this << std::endl;
//#endif //KEEP_LOG
...
void CtrlAIR_BALLIST::initConsts(quint16 SRC_amount, quint16 SRC_TRCK_amount, quint16 GT_amount)
{
...
void CtrlAIR_BALLIST::SetPersistentData(const Str_Ar_EAP_Param_AllTables &arrEAP_Tables,
...
void CtrlAIR_BALLIST::SetPersistentData(const Str_Ar_EAP_Param_AllTables &arrEAP_Tables)
{
    ...
    if (m_EAP_Tables.N_el > 0)
    {
        for (qint32 i=0; i<m_EAP_Tables.N_el; i++)
        {
            m_EAP_Tables.arData[i].BallMark_inner = ConvertBallSubcl_GDtoAirBall(m_EAP_Tables.arData[i].BallSubclass);
        }
    }
...
void CtrlAIR_BALLIST::FillCovObj(const AIR_BALL::s_arCovObj &arr_CovObj)
{
    QMutexLocker LockBallist(&m_CritSect);
...
void CtrlAIR_BALLIST::FillTrajParam(const CtrlAIR_BALL_Input_GT_Data &inp_data)
{
    m_GT_Data    = inp_data;
    m_InDataMode = AIR_BALL::GT_DATA;
    tLog(m_pLogIDs, "Mode: %d", m_InDataMode);
//    tLog(m_pLogInp, "Input GT NumGT: %d Class: %d",
//         m_GT_Data.NumbGT, m_GT_Data.Class);
    m_GT_Data.LogData(m_pLogInp);
}


void CtrlAIR_BALLIST::FillTrajParam(const CtrlAIR_BALL_Input_ST_Data &inp_data)
{
    m_ST_Data    = inp_data;
//    tLog(m_pLogInp, "Input ST SrcInd: %d NtrInSrc: %d Class: %d NumGT: %d",
//         m_ST_Data.SrcInd, m_ST_Data.NtrInSrc, m_ST_Data.Class, m_ST_Data.ID_GT);
//    LogVector(m_pLogInp, "Input Vector ST", m_ST_Data.tLoc, m_ST_Data.VectParam);
//    LogMatrix(m_pLogInp, "Input Matrix ST", inp_data.tLoc, inp_data.CovMatr);
...
//    LogMatrix(m_pLogInp, "Input Matrix ST after corr.", m_ST_Data.tLoc, m_ST_Data.CovMatr);
...
void CtrlAIR_BALLIST::FillTrajParam(const CtrlAIR_BALL_traj_param &tr_param)
{
...
void CtrlAIR_BALLIST::SetCurTime(qreal _CurTime)
{
    m_CurTime = _CurTime;
}


void CtrlAIR_BALLIST::ProcessingTrajParam(qint16 InDataMode, CtrlAIR_BALL_Output_GT_Data *pOutGT)
{
    if (pOutGT != nullptr)
    {
        pOutGT->Reset();
    ...
    try
    {
        //GLLock lockBuffer(m_CritSect, true);
...
bool CtrlAIR_BALLIST::ProcessingAirBallist(const CtrlAIR_BALL_Input_ST_Data &InData)
{
    try
    {
        QMutexLocker LockBallist(&m_CritSect);
...
bool CtrlAIR_BALLIST::ProcessingAirBallist(const CtrlAIR_BALL_Input_GT_Data &InData, CtrlAIR_BALL_Output_GT_Data &OutData)
{
    try
    {
        QMutexLocker LockBallist(&m_CritSect);
...
void CtrlAIR_BALLIST::InpMsgsProcessing()
{
...
void CtrlAIR_BALLIST::NonBallGTProcessing(qint32 ind_GT, qint32 NumGTInner)
{
...
void CtrlAIR_BALLIST::SwapNumbers(qint32 Num1, qint32 Num2)
{
...
void CtrlAIR_BALLIST::FillBallProcInData(qint32 NumGTInner)
{
    m_BallInpData.Reset();
    m_BallInpData_ST.Reset();
...
void CtrlAIR_BALLIST::FormOutputData_GT(const qint32 ind_GT)
{
...
qint16 CtrlAIR_BALLIST::ConvertBallSubcl_GDtoAirBall(const qint16 BallSubclGD)
{
...
qint16 CtrlAIR_BALLIST::ConvertBallSubcl_AirBalltoGD(const qint16 BallMarkAirBall)
{
    qint16 returnVariable;
...
qint16 CtrlAIR_BALLIST::ConvertBallPath_GDtoAirBall(const qint16 BallPathGD)
{
    qint16 returnVariable;
    switch(BallPathGD)
    {
...
qint16 CtrlAIR_BALLIST::ConvertBallPath_AirBalltoGD(const qint16 BallPathAirBall)
{
    qint16 returnVariable;
...
qint16 CtrlAIR_BALLIST::ConvertBallBranch_GDtoAirBall(const qint16 BallBranchGD)
{
    qint16 returnVariable;
    switch (BallBranchGD)
    {
...
qint16 CtrlAIR_BALLIST::ConvertBallBranch_AirBalltoGD(const qint16 BallBranchAirBall)
{
    qint16 returnVariable;
...
qint16 CtrlAIR_BALLIST::ConvertBallTType_GDtoAirBall(const qint16 BallTTypeGD)
{
    qint16 returnVariable;
    switch (BallTTypeGD)
    {
...
qint16 CtrlAIR_BALLIST::ConvertBallTType_AirBalltoGD(const qint16 BallTTypeAirBall)
{
    qint16 returnVariable;
...
qreal CtrlAIR_BALLIST::GetMaxFlightTime(const qint16 BallMarkAirBall)
{
...
qreal CtrlAIR_BALLIST::GetInputTime()
{
    qreal tLoc = 0;
    if (m_InDataMode == AIR_BALL::TEST_DATA)
    {
...
void CtrlAIR_BALLIST::ReadCellsBMMark()
{
...
bool CtrlAIR_BALLIST::IsBallistGT(const qint32 Ind_GT)
{
    bool Result = false;
...
qint32 CtrlAIR_BALLIST::GetIdGT(const qint32 NumGTExternal, const qint32 NumGTInner)
{
    qint32 id = -1;
...
qint32 CtrlAIR_BALLIST::GetInnerTrackNum(qint32 NumExt)
{
...
void CtrlAIR_BALLIST::ClearIdGT(const qint32 NumGTExt, const qint32 NumGTInner)
{
...
qint32 CtrlAIR_BALLIST::GetIdST(const qint16 SrcInd, const qint32 NtrInSrc)
{
    qint32 id = -1;
    const qreal tLoc = GetInputTime();
...
void CtrlAIR_BALLIST::ClearIdST(const qint16 SrcInd, const qint32 NtrInSrc)
{
...
qint32 CtrlAIR_BALLIST::GetIdBall(const qint32 Ind_GT, const bool bBallist)
{
    qint32 id = -1;
    const qreal tLoc = GetInputTime();
...
void CtrlAIR_BALLIST::ClearIdBall(const qint32 Ind_GT)
{
...
void CtrlAIR_BALLIST::Drop_GT(const qint32 Numb_GT, qint32 NumGTInner, const qint32 ind_Ball)
{
    ...
    m_NumGTEnumeration.DeleteTrack(Numb_GT);
}


void CtrlAIR_BALLIST::Drop_ST(const qint32 ind_ST)
{
    if (0 <= ind_ST && ind_ST < AIR_BALL::ST_FORM_AMOUNT)
    {
...
void CtrlAIR_BALLIST::ClearBallisticData(const qint32 ind_GT, const qint32 ind_Ball)
{
    if (0 <= ind_GT && ind_GT < AIR_BALL::GT_FORM_AMOUNT
...
void CtrlAIR_BALLIST::ClearInnerData_GT()
{
    for (qint32 i=0; i<AIR_BALL::GT_FORM_AMOUNT; i++)
    {
...
void CtrlAIR_BALLIST::ClearInnerData_GT_1Track(const qint32 ind_GT)
{
    if (0 <= ind_GT && ind_GT < AIR_BALL::GT_FORM_AMOUNT)
    {
