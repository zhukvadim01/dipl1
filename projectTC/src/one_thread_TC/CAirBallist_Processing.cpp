#include "CAirBallist_Processing.h"
...
namespace AIR_BALL
{
    static std::ofstream out_ball_traj_param(qPrintable(QString("%1/out_ball_traj_param.txt").arg(log_path)));  //logging trajectory parameters
    static std::ofstream out_IDs(qPrintable(QString("%1/out_ball_IDs.txt").arg(log_path)));                     //logging identificators
}
#endif //KEEP_LOG


CAirBallist_Processing::CAirBallist_Processing()
{
//#ifdef KEEP_LOG
//    AIR_BALL::out_info_ball_proc << "Constructor: CAirBallist_Processing   " << this << std::endl;
//#endif  //KEEP_LOG
//    Reset();
    InitConst();
    ClearInnerData();
...
{
//#ifdef KEEP_LOG
//    AIR_BALL::out_info_ball_proc << "Destructor: CAirBallist_Processing   " << this << std::endl;
//#endif  //KEEP_LOG
//    if (m_pLogCtrl)
//    {
//        fclose(m_pLogCtrl);
...
void CAirBallist_Processing::Reset()
{
    m_pInData = nullptr;
...
void CAirBallist_Processing::FillInputData(CAirBall_Proc_InputData_GT  &InData)
{
    m_pInData = &InData;
//    LogCtrlPoint(0, true);
}
...
void CAirBallist_Processing::FillInputData(CAirBall_Proc_InputData_ST &InData)
{
    m_pInData_ST = &InData;
//    LogCtrlPoint(0, false);
}


void CAirBallist_Processing::AirBallist_Realization()
{
...
void CAirBallist_Processing::AirBallist_Realization_ST()
{
...
//    LogCtrlPoint(3, false);
    ...
    {        
        qreal dt = m_pInData_ST->tLoc - m_arProcData.arData_ST[CurIndex].tLoc;
    ...
    if (fabs(m_pInData_ST->tLoc - m_arProcData.arData_ST[CurIndex].tLoc) > con_eps)
    {
        m_arProcData.arData_ST[CurIndex].NUpdates ++;
    }

    m_arProcData.arData_ST[CurIndex].tLoc = m_pInData_ST->tLoc;
    m_arProcData.arData_ST[CurIndex].RMSE_mean = (m_pInData_ST->CovMatr.M[0][0]
                                                + m_pInData_ST->CovMatr.M[1][1]
                                                + m_pInData_ST->CovMatr.M[2][2]) / AIR_BALL::c3;
    m_arProcData.arData_ST[CurIndex].Gamma = m_pInData_ST->Gamma;
    m_arProcData.arData_ST[CurIndex].N_StepSmooth = m_pInData_ST->N_StepSmooth;

    m_arProcData.arData_ST[CurIndex].bWellSmoothed = CurrSTIsWellSmoothed();

    CalcTrajParam(false);

    Path_Branch_Processing_ST();
//    LogCtrlPoint(4, false);
}


void CAirBallist_Processing::AirBallist_Realization_Satellite(CAirBall_Proc_OutData &OutData)
{
    OutData.Reset();
    ...
    if (0 <= IndGT && IndGT < AIR_BALL::GT_FORM_AMOUNT)
    {
                ...
                || m_pInData->tLoc - pDataGT->tPredictSatellite > c_PrognPeriod_Satellite)
        {
            ...
            m_BallProlong.FillInputData(Inp);
            m_BallProlong.CalcPolynomSatellite(ProlOut);
...
void CAirBallist_Processing::SetPersistentData(Str_Ar_EAP_Param_AllTables *p_arEAP_Tables, Str_Ar_BallCoef_Param_AllTables *p_arBallCoef_Tables)
{
...
void CAirBallist_Processing::SetPersistentData(Str_Ar_EAP_Param_AllTables *p_arEAP_Tables)
{
    m_pEAP_Tables = p_arEAP_Tables;
...
void CAirBallist_Processing::FillHeBoundaryValues(Cells_BMMark_Data *pArrCells)
{
...
void CAirBallist_Processing::SetActionPhase(const qint32 NumbGT, const qint16 ActionPhase, qreal CurTime)
{
...
void CAirBallist_Processing::SetBallCoeffSrc(const qint32 NumbGT, const qreal Gamma)
{
    if (0 < NumbGT && NumbGT < AIR_BALL::GT_FORM_AMOUNT)
    {
...
void CAirBallist_Processing::SetNumbGT(const qint32 IndGT, const qint32 NumbGT)
{
    if (0 <= IndGT && IndGT < AIR_BALL::GT_FORM_AMOUNT)
    {
        if (0 < NumbGT && NumbGT < AIR_BALL::GT_FORM_AMOUNT)
...
void CAirBallist_Processing::CalcTrajParam(const bool IsGT)
{
...
bool CAirBallist_Processing::CurrSTIsWellSmoothed()
{
    bool bRes = false;
    ...
    if (m_pInData_ST->N_StepSmooth >= AIR_BALL::cN_Well_Sm && m_arProcData.arData_ST[CurIndex].NUpdates >= AIR_BALL::cN_Well_Sm)
    {
        bRes = true;
    }
    else
...
bool CAirBallist_Processing::CurrGTIsWellSmoothed()
{
    bool bRes = true;
    ...
    {        
        ...
        if (bRes)
        {
            m_arProcData.arData_GT[GTind].tLastWellSmoothed = m_pInData->tLoc;
        }
    }
    else
    {
...
void CAirBallist_Processing::Path_Branch_Processing_GT()
{
    const qint32 CurIndex = m_pInData->IF_GT;
...
//    InpPathBranch.SetST = m_pInData->SetST;
    ...
    m_PathBranchDet.FillInputData_GT(InpPathBranch);

    m_PathBranchDet.PathBranchDet_Realization_GT();

    pGTData->tPrevPathDet = m_pInData->tLoc;
    const CPathBranch_OutputData_GT *pOutDataPBr = &m_PathBranchDet.m_OutData_PathBranch_GT; //output data from path/branch determination algorithm
...
void CAirBallist_Processing::Path_Branch_Processing_ST()
{
    const qint32 CurIndex = m_pInData_ST->IF_ST;
    ...
    {
        InpPathBranch.TypeGT = m_arProcData.arData_GT.at(m_pInData_ST->IF_GT).BallType;
        InpPathBranch.bAeroballistic = m_arProcData.arData_GT.at(m_pInData_ST->IF_GT).bAeroballistic;
    }
...
//    InpPathBranch.TrParameters.SigAh = m_TrPar.SigAh;
    ...
    m_PathBranchDet.FillInputData_ST(InpPathBranch);

    m_PathBranchDet.PathBranchDet_Realization_ST();

//    m_OutData_Ballist.P_Save_prev_PathBranchData = m_PathBranchDet.m_OutData_PathBranch.P_Save_prev_PathBranchData;
    if (!m_PathBranchDet.m_OutData_PathBranch_ST.P_Save_prev_PathBranchData)
    {
        m_arProcData.arData_ST[CurIndex].Path = m_PathBranchDet.m_OutData_PathBranch_ST.P_Path;
        m_arProcData.arData_ST[CurIndex].Branch = m_PathBranchDet.m_OutData_PathBranch_ST.P_Branch;
...
void CAirBallist_Processing::Active_Prediction_Processing()
{
    ...
    m_BallActPred.InitInput(Inp);
    m_BallActPred.GetActiveProgn(Out);
...
void CAirBallist_Processing::DeterminationBallCoeff()
{
    qint32 IndGT = m_pInData->IF_GT;
...
void CAirBallist_Processing::Prediction_Processing()
{
    //qint32 CurIndex = m_pInData->IF_GT;
    const qint32 ind_GT = m_pInData->IF_GT;
...
//        pProlOut->Reset();
        ...
        if (bStartProl)
        {
            m_BallProlong.CalcStartPoint(ProlOut);
        }
        ...
        if (bPredictConditions || ForcedProlong)
        {
            ...
            m_BallProlong.CalcFallPoint(ProlOut, ForcedProlong);
        }
        ...
        if (ProlOut.p_FallIsChanged)
        {
            ...
            {       //calculation of trapeze in the case of absence of direct threat
                ...
                m_OutData_Ballist.bNewABTrapeze = m_BallProlong.CalculateTrapezeAB();
...
void CAirBallist_Processing::TrajType_Processing()
{
    if (0 <= m_pInData->IF_BALL && m_pInData->IF_BALL < AIR_BALL::BALL_FORM_AMOUNT)
    {
        ...
        m_BallSubclDet.FillInputData_TType(InpTrajType);

        m_BallSubclDet.TrajTypeDet_Realization();

        if (m_BallSubclDet.m_OutTrajType.P_rewrite_TT)
        {
            if (m_arProcData.arData_GT[CurIndex].TrajType != m_BallSubclDet.m_OutTrajType.TrajType)
            {
                m_arProcData.arData_GT[CurIndex].TrajType = m_BallSubclDet.m_OutTrajType.TrajType;
...
void CAirBallist_Processing::BT_Mark_Processing()
{
    if (m_pInData->IF_BALL >= 0 && m_pInData->IF_BALL < AIR_BALL::BALL_FORM_AMOUNT)
    {
        ...
        m_BallSubclDet.FillInputData_BTMark(InpBTMark);

        m_BallSubclDet.BMMarkDet_Realization();

        if (m_BallSubclDet.m_OutMark.P_MarkIsChanged)
        {
            const BMMark_AndDiapason *pNewMark = &m_BallSubclDet.m_OutMark.BMMark;
            ...
            {
                m_arProcData.arData_GT[CurIndex].BMMark = m_BallSubclDet.m_OutMark.BMMark;
                m_OutData_Ballist.bNewSubcl = true;
                m_BallProlong.SetSignBMMarkChanged(m_pInData->IF_BALL);
            }
        }
...
void CAirBallist_Processing::HorizontalManeuverDetection()
{
    bool bOK = true;
...
void CAirBallist_Processing::TimeEAP_Updating()
{
    const qint32 indGT = m_pInData->IF_GT;
    if (0 <= indGT && indGT < AIR_BALL::GT_FORM_AMOUNT)
    {
...
void CAirBallist_Processing::ParametersEAP_Updating()
{
    const qint32 indGT = m_pInData->IF_GT;
    bool bOK = true;
    if (0 <= indGT && indGT < AIR_BALL::GT_FORM_AMOUNT)
...
qreal CAirBallist_Processing::GetMinActPathTimeByMark(qint32 indGT)
{
...
bool CAirBallist_Processing::CheckConditionsForPredict()
{
...
bool CAirBallist_Processing::CheckPredictionDeviation()
{
...
//bool CAirBallist_Processing::CalcDistance()
//{
//    const qint32 CurIndex = m_pInData->IF_GT;

//    if (m_arProcData.arData_GT[CurIndex].StartPoint.module() < con_par_eps
//            || m_arProcData.arData_GT[CurIndex].FallPoint.module() < con_par_eps)
//    {
//        return false;
//    }

//    CGeocentric SP_geocentr(m_arProcData.arData_GT[CurIndex].StartPoint.x,
//                            m_arProcData.arData_GT[CurIndex].StartPoint.y,
//                            m_arProcData.arData_GT[CurIndex].StartPoint.z);
//    CGeocentric FP_geocentr(m_arProcData.arData_GT[CurIndex].FallPoint.x,
//                        m_arProcData.arData_GT[CurIndex].FallPoint.y,
//                        m_arProcData.arData_GT[CurIndex].FallPoint.z);
//    CGeodesic SP_geodes;
//    CGeodesic FP_geodes;

//    GEOCENTRIC_GEO(&SP_geocentr, &SP_geodes);
//    GEOCENTRIC_GEO(&FP_geocentr, &FP_geodes);

//    CGEOLINE calcGL;
//    const bool ItsOK = calcGL.CalculationGEOLINE(&SP_geodes, &FP_geodes);

//    if (ItsOK)
//    {
//        m_arProcData.arData_GT[CurIndex].D = calcGL.m_dLength;
//    }
//    else
//    {
//        return false;
...
bool CAirBallist_Processing::Estimate_Threat(qint32 indBall)
{
    bool bThreat = false;
...
qreal CAirBallist_Processing::Estimate_AB_Distance(qint32 indGT)
{
...
void CAirBallist_Processing::NewTrackProcessing_GT()
{
    const qint32 ind_GT = m_pInData->IF_GT;
    if (0 <= ind_GT && ind_GT < AIR_BALL::GT_FORM_AMOUNT)
    {
        if (!m_arProcData.arData_GT[ind_GT].IsTaken)
        {
            ...
            m_arProcData.arData_GT[ind_GT].FirstPoint = m_pInData->Coord;
            m_arProcData.arData_GT[ind_GT].tFirst = m_pInData->tLoc;
            m_arProcData.arData_GT[ind_GT].RMSEs_FirstPoint.init(sqrt(m_pInData->CovMatr.M[0][0]),
...
void CAirBallist_Processing::NewTrackProcessing_ST()
{
...
void CAirBallist_Processing::GetFlatLoftBound_Mark(qreal &bound_flat_tr, qreal &bound_lofted_tr)
{
    bound_flat_tr = 0.;
    bound_lofted_tr = 0.;
}
...
void CAirBallist_Processing::ReturnFromPassToAct_Processing()
{
    const qint32 CurIndex = m_pInData->IF_GT;
    if (0 <= CurIndex && CurIndex < AIR_BALL::GT_FORM_AMOUNT)
    {
        CAirBall_Proc_InnerData_GT *pDataGT = &m_arProcData.arData_GT[CurIndex];
        m_BallSubclDet.ClearInnerData_Subcl_1Track(m_pInData->IF_BALL);
        m_BallSubclDet.ClearInnerData_TType_1Track(m_pInData->IF_BALL);
        pDataGT->BMMark.Reset();
        pDataGT->TrajType = AIR_BALL::UNDEFINED_TRAJ_TYPE;

        m_BallProlong.ClearInnerData_1Track(m_pInData->IF_BALL);
...
//void CAirBallist_Processing::LogCtrlPoint(const qint16 NumCtrlPoint, const bool bIsGT)
//{
...
void CAirBallist_Processing::Drop_GT(const qint32 ind_GT, const qint32 ind_Ball)
{    
    if (0 <= ind_GT && ind_GT < AIR_BALL::GT_FORM_AMOUNT)
    {
...
void CAirBallist_Processing::Drop_ST(const qint32 ind_ST)
{
...
void CAirBallist_Processing::ClearBallisticData(const qint32 ind_Ball)
{
    if (0 <= ind_Ball && ind_Ball < AIR_BALL::BALL_FORM_AMOUNT)
    {
        m_BallSubclDet.Drop_BallTrack(ind_Ball);
...
void CAirBallist_Processing::ClearInnerData()
{
...
void CAirBallist_Processing::ClearInnerData_1Track(const qint32 index)
{
...
void CAirBallist_Processing::InitConst()
{
    //prediction
    Pr_Const Const;
...
void CAirBallist_Processing::UpdateCluster()
{
    qint32 indGT = m_pInData->IF_GT;
...
void CAirBallist_Processing::ClstClearForGT(qint32 indGT, bool bGTDroped)
{
    if (0 <= indGT && indGT < AIR_BALL::GT_FORM_AMOUNT)
    {
        CAirBall_Proc_InnerData_GT *pGTData = &m_arProcData.arData_GT[indGT];
...
void CAirBallist_Processing::UpdateArrPointsForClst(qint32 ClstNum)
{
    if (0 < ClstNum && ClstNum < AIR_BALL::CLST_FORM_AMOUNT)
    {
