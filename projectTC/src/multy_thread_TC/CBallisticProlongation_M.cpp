#include "CBallisticProlongation_M.h"
...
namespace AIR_BALL
{
    static FILE* out_param = nullptr;
}
#endif //KEEP_LOG


CBallisticProlongation_M::CBallisticProlongation_M()
{
#ifdef KEEP_LOG
    AIR_BALL::out_param = OpenLog("progn_traj_M", AIR_BALL::log_path);
#endif //KEEP_LOG

    m_arProlData.resize(AIR_BALL::BALL_FORM_AMOUNT);
    try
    {
        m_pLogEll = OpenLog("Ball_EllipseParam_M", AIR_BALL::log_path);
        m_pLogCorrSP = OpenLog("StartPointCorrection_M", AIR_BALL::log_path);
    }
    catch(...)
    {
        DPS_ASSERT(false);
...
{
#ifdef KEEP_LOG
    if( AIR_BALL::out_param ) {
        fclose(AIR_BALL::out_param);
    }
#endif //KEEP_LOG
...
void CBallisticProlongation_M::Reset()
{
//    m_pInData = nullptr;
...
bool CBallisticProlongation_M::CovMatrixProlongation_FinDiff(const qreal t0, const qreal t_end, const qreal DeltaC, const qreal DeltaV, const GLPointDouble3D &Pos0, const GLPointDouble3D &Vel0, const GLPointDouble3D &Pos_end, const GLPointDouble3D &Vel_end, const TMatrix<DIM_CV> &CovCV0, const qreal Gamma, const bool P_Inverse, TMatrix<DIM_CV> &CovCVRes)
{
    bool ItsOK = true;
...
bool CBallisticProlongation_M::CovMatrixProlongation_FinDiff(const qreal t0, const qreal t_end, const qreal DeltaC, const qreal DeltaV, const GLPointDouble3D &Pos0, const GLPointDouble3D &Vel0, const TMatrix<DIM_CV> &CovCV0, const qreal Gamma, const bool P_Inverse, TMatrix<DIM_CV> &CovCVRes)
{
...
bool CBallisticProlongation_M::CovMatrixProlongation_Unscented(const qreal t0, const qreal t_end, const GLPointDouble3D &Pos0, const GLPointDouble3D &Vel0, const GLPointDouble3D &Pos_end, const GLPointDouble3D &Vel_end, TMatrix<DIM_CV> &CovCV0, const qreal Gamma, const bool P_Inverse, TMatrix<DIM_CV> &CovCVRes)
{
    qint32 jU;
...
bool CBallisticProlongation_M::CovMatrixProlongation_Unscented(const qreal t0, const qreal t_end, const GLPointDouble3D &Pos0, const GLPointDouble3D &Vel0, TMatrix<DIM_CV> &CovCV0, const qreal Gamma, const bool P_Inverse, TMatrix<DIM_CV> &CovCVRes)
{
    COnePrognosis currOneProgn;
    currOneProgn.InitConst(m_PrognConst.OnePrConsts);
    currOneProgn.TransmitPointerToTheLogFiles(m_pLogEll);
...
bool CBallisticProlongation_M::CovMatrixExtrapolation(const qreal t0, const qreal t_end,
...
void CBallisticProlongation_M::CalcStartPoint(const CBallProl_Input* pInData,
                                              CBallProl_Output &rOutData)
{
    rOutData.Reset();
    ...
    COnePrognosis_M l_OneProgn(AIR_BALL::out_param);
    l_OneProgn.TransmitPointerToTheLogFiles(m_pLogEll);
    l_OneProgn.InitConst(m_PrognConst.OnePrConsts);

    const qint32 ind_GT = pInData->IF_GT;
    const qint32 ind_ball = pInData->IF_BALL;

    rOutData.p_StartIsChanged = false;
            ...
            && 0 <= ind_ball && ind_ball < AIR_BALL::BALL_FORM_AMOUNT)
    {
        ...
        bool bCalcByDownPr = CheckConditionsForStartPointDownPr(pInData, bCounterExeeded);

        CAirBall_Proc_InnerData_GT *pGTData = &m_p_Arr_BallProc->arData_GT[ind_GT];

        if (bCalcByDownPr)
        {
            bool bOK = CalcStartPointByInvProl(pInData, l_OneProgn, rOutData);
            m_arProlData[ind_ball].tPrevTryDetStartDownPr = pInData->tLoc;
            if (!bOK)
            {
                return;
            }            
        ...
        else
        {
            ...
            {
                EstimationStartPointByReverseExtr(pInData, rOutData);
            }

            if (fabs(pInData->tLoc - m_arProlData[ind_ball].tPrevTryDetStartByTables) > con_eps)
            {
                m_arProlData[ind_ball].tPrevTryDetStartByTables = pInData->tLoc;
                          ...
                          && !m_arProlData[ind_ball].bStartByTablesPerformed ) )
                {
                    ...
                    if ( (BMMarkVal > AIR_BALL::UNKNOWN_OF_SHORT_RANGE)
                         ...
                         && (fabs(pInData->tLoc - m_arProlData[ind_ball].tDetStartDownPr) <= c_dT_def_start_2method
                             || (fabs(m_arProlData[ind_ball].tDetStartDownPr - AIR_BALL::Timer_INI) < con_par_eps
                                 && (fabs(pInData->tLoc - pGTData->tEAP) <= c_dT_def_start_2method)))
                         && (m_pEAP_Tables != nullptr))
                    {
                        Str_Ar_EAP_Param_1Table* pEAP_table = m_pEAP_Tables->getCellMark(BMMarkVal);
                        if (pEAP_table != nullptr)
                        {
                            CalcStartPointByTable(pInData, l_OneProgn, rOutData, pEAP_table, true);
                        }
                    }
                }
                          ...
                          && !m_arProlData[ind_ball].bStartByTablesPerformed_multival) )
                {
                         ...
                         && (m_pEAP_Tables != nullptr))
                    {
                        ...
                        for (qint16 i = pCurMark->minBMMark; i<= pCurMark->maxBMMark; i++)
                        {
                            ...
                            if (pEAP_table != nullptr)
                            {
                                CalcStartPointByTable(pInData, l_OneProgn, rOutData, pEAP_table, false);
                            }
                        }
                    }
                }
        ...
        if (rOutData.p_StartIsChanged)
        {
            //correction of start point
            CorrectionStartPoint(pInData);
...
bool CBallisticProlongation_M::CalcStartPointByInvProl(const CBallProl_Input *pInData,
                                                       COnePrognosis_M &rOneProgn,
                                                       CBallProl_Output& rOutData)
{
    bool bRes = false;
    const qint32 ind_GT = pInData->IF_GT;
                   ...
                   || m_p_Arr_BallProc->arData_GT[ind_GT].Path == AIR_BALL::PASSIVE_PATH))
    {
        ...
        const qreal KGamma = CalcKGamma(pInData->H);

        rOneProgn.Reset();
        //reverse prolongation
        rOneProgn.CalcTrack(pInData->tLoc, pInData->Coord.x, pInData->Coord.y, pInData->Coord.z,
                            - pInData->Vel.x, - pInData->Vel.y, - pInData->Vel.z,
                            Gamma * KGamma, true);
        std::vector<M10> ProlTrack;
        qint32 dimension = 0;
        rOneProgn.GetProlongedTrack(ProlTrack, dimension);
        if (!ProlTrack.empty() && rOneProgn.GetSignCorrectProgn())
        {
            if (m_p_Arr_BallProc->arData_GT[ind_GT].Branch == AIR_BALL::DESCENDING_BRANCH) //m_OneProgn.GetHmax() > m_arProlData[ind_ball].Hapogee)
            {
                m_arProlData[ind_ball].Hapogee = rOneProgn.GetHmax();
                qreal tApogeeNew = AIR_BALL::c2 * pInData->tLoc - rOneProgn.GetTApogee();
            ...
            qint32 size = 0;
            qreal Tprol = 0.;
            bool ItsOK = rOneProgn.OutputLastPoint(Tprol, size, CoordSP, VelSP);
            ...
            ItsOK = CalculateEllipse(m_p_Arr_BallProc->arData_GT[ind_GT].BMMark.MarkValue,
                                     ...
                                     pInData->tLoc, Tprol, const_cast<GLMatrix&>(pInData->CovMatr),
                                     pInData->Coord, pInData->Vel, CoordSP, VelSP,
                                     pInData->Sign_large_load, Gamma * KGamma, true, EllInfo, CovSP);
            if (!ItsOK)
            {
                return false;
            }

            qreal tStart = 2.* pInData->tLoc - Tprol;

            bool bCheckOK = CheckStartPoint(pInData, tStart, CoordSP, CovSP);
            if (!bCheckOK)
            {
            ...
            if (ItsOK)
            {
                ...
                for (qint32 j=1; j<Size; j++)
                {
                    ProlTrack[j].M_info[0] = 2. * pInData->tLoc - ProlTrack[j].M_info[0];
                ...
                m_arProlData[ind_ball].StartPointFindMethod = AIR_BALL::START_POINT_BY_DOWN_PROGNOSIS;
                m_arProlData[ind_ball].tDetStartDownPr = pInData->tLoc;
                ...
                rOutData.p_StartIsChanged = true;
                bRes = true;
            }
        }
    }
...
void CBallisticProlongation_M::CalcStartPointByTable(const CBallProl_Input* pInData,
                                                     COnePrognosis_M& rOneProgn,
                                                     CBallProl_Output& rOutData,
                                                     Str_Ar_EAP_Param_1Table *pEAP_table,
                                                     bool bSingleValued)
{
    if (pEAP_table == nullptr) {
        return;
    ...
    ItsOK = rOneProgn.FindStartByEAP(pEAP_table, pInData->tLoc,
                                     pInData->Coord.x, pInData->Coord.y, pInData->Coord.z,
                                     -pInData->Vel.x, -pInData->Vel.y, -pInData->Vel.z,
    ...
    if (ItsOK && (bSingleValued || bKeepData_multi)) //EAP is found
    {
        ...
        ItsOK = rOneProgn.OutputStartPointByTables(pInData->tLoc, pEAP_table,
                                                   StartTime, CoordSP, T_passive, CoordEAP, VelEAP);

        qreal LCurrFst = pInData->Coord.getDistanceOnSurface(pGTData->FirstPoint);
        qreal LCurrStart = pInData->Coord.getDistanceOnSurface(CoordSP);
        ...
        ItsOK = CalculateEllipse(pGTData->BMMark.MarkValue,
                                 pGTData->TrajType,
                                 pInData->tLoc, pInData->tLoc+T_passive, const_cast<GLMatrix&>(pInData->CovMatr),
                                 pInData->Coord, pInData->Vel*(-1.),
                                 CoordEAP, VelEAP, pInData->Sign_large_load,
                                 Gamma, true, EllInfo);
        ...
        qreal tEAP_by_table = pInData->tLoc - T_passive;
        CorrInverseProlTrack(pInData, true, tEAP_by_table, CoordEAP);
...
//            GLPointDouble3D P_EAP(TrackFragment[0].M_info[1], TrackFragment[0].M_info[2], TrackFragment[0].M_info[3]);
            bool bOK = CalcSepBoostPoint(StartTime, pEAP_table->t_SepBoost1, CoordSP, CoordEAP,
                                         ResHBoost1, ResLBoost1, ResVBoost1, rOutData.PointSepBoost1);
            if (bOK)
            {
                rOutData.bBoostSepPointsChanged = true;
                ...
                if (ResHBoost2 > con_eps && fabs(ResLBoost2) > con_eps && ResVBoost2 > con_eps)
                {
                    bOK = CalcSepBoostPoint(StartTime, pEAP_table->t_SepBoost2, CoordSP, CoordEAP,
                                            ResHBoost2, ResLBoost2, ResVBoost2, rOutData.PointSepBoost2);
                }
            }
        }
...
void CBallisticProlongation_M::EstimationStartPointByReverseExtr(const CBallProl_Input *pInData,
                                                                 CBallProl_Output &rOutData)
{
    const qint32 indGT = pInData->IF_GT;
    const qint32 indBall = pInData->IF_BALL;
            ...
            && 0 <= indBall && indBall < AIR_BALL::BALL_FORM_AMOUNT)
    {
        ...
        GLPointDouble3D VelSP = pInData->Vel;
        ...
        else
        {
            ...
            fCalc.EstimateTimeEarthSurfaceIntersection(pInData->Coord, pInData->Vel, pInData->Acc, T1c, T2c, T3c, T4c);
            ...
            {
                rOutData.p_StartIsChanged = true;
                m_arProlData[indBall].tStart = pInData->tLoc + dtRes;
                m_arProlData[indBall].StartPoint = pInData->Coord + (pInData->Vel*dtRes) + (pInData->Acc*sqr(dtRes)/2.);
                VelSP = pInData->Vel + (pInData->Acc*dtRes);
            }
            else //intersection is not found
            {
                //extrapolation using coordinates and velocities
                fCalc.EstimateTimeEarthSurfaceIntersection(pInData->Coord, pInData->Vel, T1c, T2c);
                CompareTime(T1c, dtRes);
                CompareTime(T2c, dtRes);
                if (dtRes < - con_eps)
                {
                    rOutData.p_StartIsChanged = true;
                    m_arProlData[indBall].tStart = pInData->tLoc + dtRes;
                    m_arProlData[indBall].StartPoint = pInData->Coord + (pInData->Vel*dtRes);
                    VelSP = pInData->Vel;
                }
            }
        }
        ...
        if (rOutData.p_StartIsChanged)
        {
            ...
            TCMatrixFunc<DIM_CV> fMatr;
            fMatr.Copy(CovCV0, const_cast<GLMatrix&>(pInData->CovMatr), 0, DIM_CV-1, 0, DIM_CV-1);
            bool bOK = CovMatrixExtrapolation(pInData->tLoc, m_arProlData[indBall].tStart, CovCV0, CovCVRes);
            if (bOK)
            {
                ...
                m_arProlData[indBall].tDetStartDownPr = pInData->tLoc;
            }
...
void CBallisticProlongation_M::CompareTime(GLComplexNumb &dTcompl, qreal &dTreal)
{
...
void CBallisticProlongation_M::UpdateApogeeTime(qint32 ball_ind, qreal tApogeeNew)
{
...
void CBallisticProlongation_M::CalcFallPoint(const CBallProl_Input *pInData,
                                             CBallProl_Output &rOutData,
                                             const bool ForcedProlong)
{
    if (m_p_Arr_BallProc == nullptr) {
        return;
    }
    ...
    rOutData.p_FallIsChanged = false;
    rOutData.SignProgn = AIR_BALL::OLD_PROGNOSIS;

    const qint32 ind_ball = pInData->IF_BALL;
    const qint32 ind_GT = pInData->IF_GT;
    const qreal PrPeriod = GetProlPeriod(pInData);

    if (pInData->tLoc - m_arProlData[ind_ball].tSavedProl > PrPeriod
            || ForcedProlong)
    {
        const qreal Gamma = m_p_Arr_BallProc->arData_GT[ind_GT].Gamma;

        if (!pInData->bAeroballistic)
        {
            l_OneProgn.CalcTrack(pInData->tLoc, pInData->Coord.x, pInData->Coord.y, pInData->Coord.z,
                                 pInData->Vel.x, pInData->Vel.y, pInData->Vel.z, Gamma, false);
        }
        else //aeroballistic missile
        {
            ...
            bool bOK_Ab = l_OneProgn.CalcTrack_Aeroball(pInData->tLoc, t0_Aeroball,
                                                        pInData->Coord.x, pInData->Coord.y, pInData->Coord.z,
                                                        pInData->Vel.x, pInData->Vel.y, pInData->Vel.z, Gamma, Delta);
            if (!bOK_Ab)
            {
                l_OneProgn.CalcTrack(pInData->tLoc, pInData->Coord.x, pInData->Coord.y, pInData->Coord.z,
                                     pInData->Vel.x, pInData->Vel.y, pInData->Vel.z, Gamma, false);
        ...
        if (m_p_Arr_BallProc->arData_GT[ind_GT].Branch == AIR_BALL::ASCENDING_BRANCH
                && !pInData->bAeroballistic)
        {
            m_arProlData[ind_ball].Hapogee = l_OneProgn.GetHmax();
            qreal tApogeeNew = l_OneProgn.GetTApogee();
            UpdateApogeeTime(ind_ball, tApogeeNew);
        }        

        if (!l_OneProgn.GetSignCorrectProgn())
        {
            rOutData.SignProgn = AIR_BALL::INCORRECT_PROGNOSIS;
            return;
        }
        ...
        qint32 size = 0;
        qreal Tprol = 0.;
        bool ItsOK = l_OneProgn.OutputLastPoint(Tprol, size, CoordFP, VelFP);
        if (!ItsOK) {
            return;
        }
        ...
        std::vector<M10> ProlTrack;
        qint32 dimension = 0;
        l_OneProgn.GetProlongedTrack(ProlTrack, dimension);
        GlueProlongedTrack(pInData, rOutData, ProlTrack); //gluing prolonged track

        ItsOK = l_OneProgn.GetPolinom(ProlTrack, m_arProlData[ind_ball].PolData); //calculate polinom
        if (ItsOK) {
            rOutData.Polynoms = m_arProlData[ind_ball].PolData;
        }

        CalcOutPoints(pInData->IF_GT, pInData->IF_BALL, rOutData); //calculate 50 points of trajectory

        //Calculation of dispersion ellipse in fall point
        qreal EllPrognPeriod; //period of ellipse recalculation
        if (!pInData->SignQuickReaction)
        {
        ...
        if (pInData->tLoc - m_arProlData[ind_ball].tSavedEll > EllPrognPeriod)
        {
            EllipseInfo EllInfo;
            ItsOK = CalculateEllipse(m_p_Arr_BallProc->arData_GT[ind_GT].BMMark.MarkValue,
                                     m_p_Arr_BallProc->arData_GT[ind_GT].TrajType,
                                     pInData->tLoc, Tprol, const_cast<GLMatrix&>(pInData->CovMatr),
                                     pInData->Coord, pInData->Vel,
                                     CoordFP, VelFP, pInData->Sign_large_load,
                                     Gamma, false, EllInfo);
            if (ItsOK)
            {
                rOutData.p_FallEllIsChanged = true;
                m_arProlData[ind_ball].FallEll = EllInfo;
                m_arProlData[ind_ball].tSavedEll = pInData->tLoc;
            }
        }

        //fall point storage
        m_arProlData[ind_ball].tSavedProl = pInData->tLoc;
        m_arProlData[ind_ball].N_Prol ++;
        m_arProlData[ind_ball].FallPoint = CoordFP;
        m_arProlData[ind_ball].tFall = Tprol;
        m_arProlData[ind_ball].D = CalcDistance(pInData->IF_BALL);
        rOutData.p_FallIsChanged = true;
        if (rOutData.bArrPointsFilled && rOutData.SignProgn != AIR_BALL::INCORRECT_PROGNOSIS)
        {
            rOutData.SignProgn = AIR_BALL::NEW_PROGNOSIS;
        }
        ...
        m_arProlData[ind_ball].SignLastProgn = rOutData.SignProgn;
        ...
        {
            const bool isFirst = rOutData.Polynoms.LogPolynom();
            if (isFirst) {
                l_OneProgn.OutputTrack();
            }
        }
    }
}
...
bool CBallisticProlongation_M::CalcSepBoostPoint(qreal t_start, qreal dt_sep, GLPointDouble3D SP, GLPointDouble3D P_EAP,
...
void CBallisticProlongation_M::CalcPolynomSatellite(const CBallProl_Input *pInData,
                                                    CBallProl_Output &rOutData)
{
    rOutData.Reset();
    ...
    qreal tProgn = pInData->tLoc + c_TimeProlSatellite;
    bool bOK = l_OneProgn.CalcTrack_OnTime(pInData->tLoc, tProgn, pInData->Coord.x, pInData->Coord.y, pInData->Coord.z,
                                           pInData->Vel.x, pInData->Vel.y, pInData->Vel.z, c_Gamma_st, false);
    if (bOK && l_OneProgn.GetSignCorrectProgn())
    {
        std::vector<M10> ProlTrack;
        qint32 dimension = 0;
        l_OneProgn.GetProlongedTrack(ProlTrack, dimension);
        if (dimension > 1)
        {
            bOK = l_OneProgn.GetPolinom(ProlTrack, rOutData.Polynoms);
            if (bOK)
            {
                rOutData.SignProgn = AIR_BALL::NEW_PROGNOSIS;
            }
...
bool CBallisticProlongation_M::CheckConditionsForStartPointDownPr(const CBallProl_Input *pInData,
                                                                  bool &bCounterExceeded)
{
    bool bRes = false;
            ...
            && 0 <= ind_ball && ind_ball < AIR_BALL::BALL_FORM_AMOUNT)
    {
        ...
        {
            if (fabs(pInData->tLoc - m_arProlData[ind_ball].tPrevTryDetStartDownPr) > con_eps)
            {
                if (m_p_Arr_BallProc->arData_GT[ind_GT].Branch == AIR_BALL::ASCENDING_BRANCH
...
void CBallisticProlongation_M::GetStartPoint(const qint32 ind_ball, GLPointDouble3D &StartPoint, qreal &StartTime)
{    
    StartPoint.clear();
    StartTime = 0.;
    if (ind_ball >= 0)
...
void CBallisticProlongation_M::GetFallPoint(const qint32 ind_ball, GLPointDouble3D &FallPoint, qreal &FallTime)
{
    if (ind_ball >= 0)
    {
...
bool CBallisticProlongation_M::CalculateEllipse(const qint16 BMMarkValue, const qint16 TrajType,
                                                const qreal CurrTime, const qreal EndTime,
                                                TMatrix<SIZE_M> &CovMatr,
                                                const GLPointDouble3D &CurrPoint, const GLPointDouble3D &CurrVel,
                                                const GLPointDouble3D &EndPoint, const GLPointDouble3D &EndVel,
                                                const bool SignLargeLoad, const qreal Gamma, const bool SignInvProgn, EllipseInfo &EllInfo)
{
    TMatrix<DIM_CV> CovCVRes;
    bool bRes = CalculateEllipse(BMMarkValue, TrajType, CurrTime, EndTime,
                                 CovMatr, CurrPoint, CurrVel, EndPoint, EndVel,
                                 SignLargeLoad, Gamma, SignInvProgn, EllInfo, CovCVRes);
    return bRes;
}


bool CBallisticProlongation_M::CalculateTrapezeAB(const CBallProl_Input *pInData,
                                                  CBallProl_Output& rOutData)
{
    const qint32 ind_Ball = pInData->IF_BALL;
    ...
    //add Evgenia
    bool ok;
    CGeocentric currpointECEF(pInData->Coord.x,
                              pInData->Coord.y,
                              pInData->Coord.z); //current point in ECEF
    ...
    rOutData.AB_Trapeze.MinRange = r1;
    rOutData.AB_Trapeze.MaxRange = std::max(r2,pInData->AB_MaxDistance);
    rOutData.AB_Trapeze.P_base.x = currpointprojECEF.m_dX;
    rOutData.AB_Trapeze.P_base.y = currpointprojECEF.m_dY;
    rOutData.AB_Trapeze.P_base.z = currpointprojECEF.m_dZ;
    rOutData.AB_Trapeze.AzFly = azflu * con_180_div_PI; //radians --> degrees
    rOutData.AB_Trapeze.SStrAzFly = alpha * con_180_div_PI; //radians --> degrees
    rOutData.AB_Trapeze.SigmaAzFly = (alpha/3.) * con_180_div_PI; //radians --> degrees

    return ok;
}


void CBallisticProlongation_M::CorrectionStartPoint(const CBallProl_Input *pInData)
{
    const qint32 ind_GT = pInData->IF_GT;
    const qint32 ind_ball = pInData->IF_BALL;
    ...
    if (H1st <= cHeight4StartCheck && SignFstP) //BM was detected close to the launch
    {
        ...
        if (H1st <= cHeight4Start && bOK) //BM was detected very close to the launch
        {
            ...
            if (bNeedCorrection)
            {
                ...
                tLog(m_pLogCorrSP, "NumGT: %d IndGT: %d tLoc: %f H0: %f",
                     pGTData->NumbGT, ind_GT, pInData->tLoc, H1st);
        ...
        {
            CGeocentric CurrP_ECEF(pInData->Coord.x, pInData->Coord.y, pInData->Coord.z),
...
bool CBallisticProlongation_M::CheckStartPoint(const CBallProl_Input *pInData, const qreal &tStart,
                                               GLPointDouble3D &SP, TMatrix<DIM_CV> &CovSP)
{
...
void CBallisticProlongation_M::SetSignBMMarkChanged(const qint32 ind_Ball)
{
...
void CBallisticProlongation_M::GetOutPoints(qint32 GTInd, qint32 ind_ball,
                                            CBallProl_Output &rOutData)
{
    CalcOutPoints(GTInd, ind_ball, rOutData);
}


void CBallisticProlongation_M::Drop_BallTrack(const qint32 ind_Ball)
{
    if (0 <= ind_Ball && ind_Ball < AIR_BALL::BALL_FORM_AMOUNT)
    {
...
void CBallisticProlongation_M::SetPersistentData(Str_Ar_EAP_Param_AllTables *p_arEAP_Tables, Str_Ar_BallCoef_Param_AllTables *p_arBallCoef_Tables)
{
...
void CBallisticProlongation_M::SetPersistentData(Str_Ar_EAP_Param_AllTables *p_arEAP_Tables)
{
    m_pEAP_Tables = p_arEAP_Tables;
}


void CBallisticProlongation_M::InitConst(const Pr_Const &Const)
{
    m_PrognConst = Const;
...
void CBallisticProlongation_M::InitPointerToBallProcArray(CAirBall_Proc_Inner_Arr *Arr)
{
    m_p_Arr_BallProc = Arr;
}


void CBallisticProlongation_M::GlueProlongedTrack(const CBallProl_Input *pInData,
                                                  CBallProl_Output &rOutData,
                                                  const  std::vector<M10> &NewProlTrack)
{
    const qint32 ind_ball = pInData->IF_BALL;
    if (ind_ball < 0)
    {
    ...
    {        
        ...
        qreal t_corr = pInData->tLoc - m_PrognConst.dT_corr_progn; //time at which correction begins
        ...
        for (j=0; j<SizeOldProl-1; j++)
        {
            ...
            if (    m_arProlData[ind_ball].ProlongedTrack[j].M_info[0] - pInData->tLoc < con_par_eps
                 && m_arProlData[ind_ball].ProlongedTrack[j+1].M_info[0] - pInData->tLoc > -con_par_eps)
        ...
        if (Ai == -1)
        {
            if (pInData->tLoc > m_arProlData[ind_ball].ProlongedTrack[SizeOldProl-1].M_info[0])
            {
                Ai = SizeOldProl - 1;
            }
            else
        ...
        if (Ai > 0)
        {
            ...
            qint32 j_dt0 = Ai;
            if (pInData->tLoc - m_arProlData[ind_ball].ProlongedTrack[Ai].M_info[0] < con_par_eps)
            {
                j_dt0 = Ai-1;
            }
            const qreal dT0 = pInData->tLoc - m_arProlData[ind_ball].ProlongedTrack[j_dt0].M_info[0];
            const qreal dT = pInData->tLoc - t_corr;
            ...
            const qreal dX_t = (pInData->Coord.x - Xextr) / dT;
            const qreal dY_t = (pInData->Coord.y - Yextr) / dT;
            const qreal dZ_t = (pInData->Coord.z - Zextr) / dT;
...
void CBallisticProlongation_M::GetDeltaFinDiff(const qint16 BMMark, const qint16 TrajType, qreal &DeltaCoord, qreal &DeltaVel)
{
...
qreal CBallisticProlongation_M::GetProlPeriod(const CBallProl_Input *pInData)
{
    qreal PrPeriod = 0.;

    if (( pInData->ActionPhase == KILL_ASSESSMENT
          || pInData->ActionPhase == ASSESSMENT_RESULT
          || pInData->ActionPhase == MEET_POINT
          || pInData->ActionPhase == DESTROYED
          || pInData->ActionPhase == MISSED)
            && fabs(pInData->tLoc - pInData->tObtainActionPhase) < c_ControlTimeNearMeet)
    {
        PrPeriod = c_PrognPeriod_NearMeet;
    }
    else
    {
        if (pInData->BallType == SUBCLASS_WARHEAD /*|| m_pInData->BallType == TYPE_MaCRV*/) //BO is warhead
        {
            if (pInData->bAeroballistic) //aeroballistic missile
            {
                if (pInData->Sign_large_load) //large load
                {
            ...
            else if (!pInData->SignQuickReaction) //sufficient margin of time
            {
                if (pInData->Sign_large_load) //large load
                {
...
qreal CBallisticProlongation_M::CalcKGamma(qreal H)
{
...
void CBallisticProlongation_M::CorrInverseProlTrack_old(const CBallProl_Input *pInData,
                                                        const M10 &SP, const std::vector<M10> &TrackFragment)
{
    const qint32 ind_ball = pInData->IF_BALL; //index of ballistic trajectory
    qint32 i;
...
void CBallisticProlongation_M::CorrInverseProlTrack(const CBallProl_Input *pInData,
                                                    bool bNewEAP, qreal tNewEAP,
                                                    GLPointDouble3D PointNewEAP)
{
    qint32 GTInd = pInData->IF_GT;
    qint32 BallInd = pInData->IF_BALL;
    bool bOK = true;
            ...
            && 0 <= BallInd && BallInd < AIR_BALL::BALL_FORM_AMOUNT)
    {
        ...
        qreal t_attachment = pInData->tLoc;
        GLPointDouble3D PointAttachment = pInData->Coord;
                ...
                && Size > 0)
        {
            ...
            CorrInverseProlTrack_MoveToPoint(BallInd, pBallData->StartPoint, t0, tFix, pBallData->tStart, true);
                    ...
                    && bOK)
            {
                ...
                CorrInverseProlTrack_MoveToPoint(BallInd, PointFix, tFix, t_attachment, tFix, false);
                        ...
                        && !pGTData->ManPoint.IsZero())
                {
                    ...
                    CorrInverseProlTrack_MoveToPoint(BallInd, PointFix, tFix2, tFix, tFix2, false);
                    tFix = tFix2;
                }

                //attachment track to the start point
                CorrInverseProlTrack_MoveToPoint(BallInd, pBallData->StartPoint, pBallData->tStart,
                                                 tFix, pBallData->tStart, false);
            }
        }
    }
}
...
void CBallisticProlongation_M::CorrInverseProlTrack_old1(const qint32 ind_ball,
                                                         const M10 &SP, const std::vector<M10> &TrackFragment)
{
    ...
    if(t_sp > OldTrack[0].M_info[0] || fabs(t_sp - OldTrack[0].M_info[0]) < con_eps)
    {
        ...
        qreal dx_sp = SP.M_info[1] - OldTrack[Loc_SP].M_info[1];
        qreal dy_sp = SP.M_info[2] - OldTrack[Loc_SP].M_info[2];
        qreal dz_sp = SP.M_info[3] - OldTrack[Loc_SP].M_info[3];

        qreal dx_0 = TrackFragment[0].M_info[1] - OldTrack[Loc0].M_info[1];
        qreal dy_0 = TrackFragment[0].M_info[2] - OldTrack[Loc0].M_info[2];
        qreal dz_0 = TrackFragment[0].M_info[3] - OldTrack[Loc0].M_info[3];

        qreal dx=0, dy=0, dz=0;
        ...
        for(qint32 i=Loc_SP+1; i<Loc0; i++)
        {
            ...
            qreal dt = NewPoint.M_info[0] - NewTrack[SizeNew-1].M_info[0];
    ...
    {
        bResOK = CorrInverseProlTrackTStartLessTOld(ind_ball, SP, OldTrack, TrackFragment);
    }
    else {
    }
...
bool CBallisticProlongation_M::CorrInverseProlTrackTStartLessTOld(const qint32 ind_ball,
                                                                  const M10 &SP, const std::vector<M10> &OldTrack,
                                                                  const std::vector<M10> &TrackFragment)
{
    bool bResOK = false;
...
void CBallisticProlongation_M::CorrInverseProlTrack_MoveToPoint(qint32 BallInd,
                                                                GLPointDouble3D &BasePoint, qreal t_0, qreal t_end,
                                                                qreal t_0_base, bool bTimeCorr)
{
...
qint16 CBallisticProlongation_M::CalculateNforT(qreal tN, const std::vector<M10> &OldTrack)
{
    qint16 iN=0;
    const qint32 SizeOldTrack = OldTrack.size();
    qint32 i;
...
void CBallisticProlongation_M::CalcOutPoints(qint32 GTInd, qint32 ind_ball,
                                             CBallProl_Output &rOutData)
{
    rOutData.arrPoints.Reset();
            ...
            && 0 <= GTInd && GTInd < AIR_BALL::GT_FORM_AMOUNT)
    {
        ...
        if (bFromStart)
        {
            ...
            rOutData.arrPoints[0].time_point = t0; //initial point
            rOutData.arrPoints[0].P.x = ProlTrack[0].M_info[1];
            rOutData.arrPoints[0].P.y = ProlTrack[0].M_info[2];
            rOutData.arrPoints[0].P.z = ProlTrack[0].M_info[3];
            rOutData.arrPoints[0].V.x = ProlTrack[0].M_info[4];
            rOutData.arrPoints[0].V.y = ProlTrack[0].M_info[5];
            rOutData.arrPoints[0].V.z = ProlTrack[0].M_info[6];
            rOutData.arrPoints[0].A.x = ProlTrack[0].M_info[7];
            rOutData.arrPoints[0].A.y = ProlTrack[0].M_info[8];
            rOutData.arrPoints[0].A.z = ProlTrack[0].M_info[9];
            i_begin = 1;
        }
        else
        {
            ...
            if (bJoinIniTrackFromClst)
            {
                ...
                for (it = pClstData->InitialArrPoints.begin(); it != pClstData->InitialArrPoints.end(); it++)
                {
                    if (i < AIR_BALL::N_OUT_POINTS)
                    {
                        ...
                        rOutData.arrPoints[i] = CurrPoint;
                        i++;
                    }
                }
                i_begin = i;
        ...
        for (qint16 i0=0; i0<N_Points_curr; i0++)
        {
            ...
            rOutData.arrPoints[i].time_point = cur_t;
            ...
            {
                rOutData.arrPoints[i].P.x = ProlTrack[k].M_info[1];
                rOutData.arrPoints[i].P.y = ProlTrack[k].M_info[2];
                rOutData.arrPoints[i].P.z = ProlTrack[k].M_info[3];
                rOutData.arrPoints[i].V.x = ProlTrack[k].M_info[4];
                rOutData.arrPoints[i].V.y = ProlTrack[k].M_info[5];
                rOutData.arrPoints[i].V.z = ProlTrack[k].M_info[6];
                rOutData.arrPoints[i].A.x = ProlTrack[k].M_info[7];
                rOutData.arrPoints[i].A.y = ProlTrack[k].M_info[8];
                rOutData.arrPoints[i].A.z = ProlTrack[k].M_info[9];
            }
            else
            {
                const qreal rel_T = (cur_t - ProlTrack[k-1].M_info[0])
                        / (ProlTrack[k].M_info[0] - ProlTrack[k-1].M_info[0]);
                rOutData.arrPoints[i].P.x = ProlTrack[k-1].M_info[1]
                        + rel_T * (ProlTrack[k].M_info[1] - ProlTrack[k-1].M_info[1]);
                rOutData.arrPoints[i].P.y = ProlTrack[k-1].M_info[2]
                        + rel_T * (ProlTrack[k].M_info[2] - ProlTrack[k-1].M_info[2]);
                rOutData.arrPoints[i].P.z = ProlTrack[k-1].M_info[3]
                        + rel_T * (ProlTrack[k].M_info[3] - ProlTrack[k-1].M_info[3]);
                rOutData.arrPoints[i].V.x = ProlTrack[k-1].M_info[4]
                        + rel_T * (ProlTrack[k].M_info[4] - ProlTrack[k-1].M_info[4]);
                rOutData.arrPoints[i].V.y = ProlTrack[k-1].M_info[5]
                        + rel_T * (ProlTrack[k].M_info[5] - ProlTrack[k-1].M_info[5]);
                rOutData.arrPoints[i].V.z = ProlTrack[k-1].M_info[6]
                        + rel_T * (ProlTrack[k].M_info[6] - ProlTrack[k-1].M_info[6]);
                rOutData.arrPoints[i].A.x = ProlTrack[k-1].M_info[7]
                        + rel_T * (ProlTrack[k].M_info[7] - ProlTrack[k-1].M_info[7]);
                rOutData.arrPoints[i].A.y = ProlTrack[k-1].M_info[8]
                        + rel_T * (ProlTrack[k].M_info[8] - ProlTrack[k-1].M_info[8]);
                rOutData.arrPoints[i].A.z = ProlTrack[k-1].M_info[9]
                        + rel_T * (ProlTrack[k].M_info[9] - ProlTrack[k-1].M_info[9]);
            }
        }
        rOutData.bArrPointsFilled = true;
...
void CBallisticProlongation_M::SubstituteOutPoint(const qreal &t0, GLPointDouble3D &Point0,
                                                  CBallProl_Output &rOutData)
{
    if (fabs(t0 - AIR_BALL::Timer_INI) > con_eps && !Point0.IsZero())
    {
        qint32 IndSubst = rOutData.GetNearIndex(t0);
        if (IndSubst > 0 && IndSubst < AIR_BALL::N_OUT_POINTS - 1)
        {
            rOutData.arrPoints[IndSubst].time_point = t0;
            rOutData.arrPoints[IndSubst].P = Point0;
        }
    }
}


qreal CBallisticProlongation_M::CalcDistance(const qint32 ind_ball)
{
    qreal d = -1;
    bool bOK = true;
...
void CBallisticProlongation_M::ClearInnerData()
{
    //for (qint32 i=0; )
...
void CBallisticProlongation_M::ClearInnerData_1Track(const qint32 ind_Ball)
{
    if (0 <= ind_Ball && ind_Ball < AIR_BALL::BALL_FORM_AMOUNT)
    {
