...
#include "CPathBranchDetermination_M.h"
#include "air_ballist_literal.h"

// using namespace std;
using namespace GLFileLog;

static const qreal SigmaA0 = 2. * cK_VT
        * sqrt((pow(cEAP_Ksi, 2.*static_cast<qreal>(cEAP_m0) - 2.)*(3.*cEAP_Ksi-1.)
                + sqr(cEAP_Ksi) - 2.*cEAP_Ksi + 1) / (cEAP_Ksi + 1.));
static const qreal SigmaA1 = 2. * cK_VT
        * sqrt((pow(cEAP_Ksi, 2.*(static_cast<qreal>(cEAP_m0)+8.) - 2.)*(3.*cEAP_Ksi-1.)
                + sqr(cEAP_Ksi) - 2.*cEAP_Ksi + 1) / (cEAP_Ksi + 1.));

CPathBranchDetermination_M::CPathBranchDetermination_M()
{
//#ifdef KEEP_LOG
//    AIR_CLS::out_info_path_branch << "Constructor: CPathBranchDetermination_M  " << this << std::endl;
//#endif //KEEP_LOG
    ...
    try
    {
        m_pLogPBr = OpenLog("out_PathBranch_M", AIR_BALL::log_path);
    }
    catch(...)
    {
        DPS_ASSERT(false);
...
CPathBranchDetermination_M::~CPathBranchDetermination_M()
{
    ...
    m_pLogPBr = nullptr;
//#ifdef KEEP_LOG
//    AIR_CLS::out_info_path_branch << "Destructor: CPathBranchDetermination_M  " << this << std::endl;
//#endif //KEEP_LOG
}


void CPathBranchDetermination_M::Reset()
{
...
//#ifdef KEEP_LOG
//    AIR_CLS::out_info_path_branch << "Reset CPathBranchDetermination_M  " << this << std::endl;
//#endif //KEEP_LOG
}


void CPathBranchDetermination_M::InitPointerToBallProcArray(CAirBall_Proc_Inner_Arr *Arr)
{
    m_p_Arr_BallProc = Arr;
}


void CPathBranchDetermination_M::PathBranchDet_Realization_ST(const CPathBranch_InputData_ST* pInData,
                                                              CPathBranch_OutputData_ST& rOutData )
{
    rOutData.Reset();

    const qint32 CurIndex = pInData->IF_ST; //index of current general track
    const qint16 PrevPath = m_arPathBrData_ST[CurIndex].P_Path;
    const qint16 PrevBranch = m_arPathBrData_ST[CurIndex].P_Branch;

    VhArrayFilling(pInData);

    if (m_arPathBrData_ST[CurIndex].P_Branch == AIR_BALL::DESCENDING_BRANCH)
    {
        Branch_Check_ST(pInData);
    } else {
        Branch_Determination_ST(pInData);
    }
    ...
    if (m_arPathBrData_ST[CurIndex].P_Branch == AIR_BALL::DESCENDING_BRANCH)
    {
                ...
                || (m_arPathBrData_ST[CurIndex].P_Path == AIR_BALL::ACTIVE_PATH))
        {
            ...
            m_arPathBrData_ST[CurIndex].T_def_eap = pInData->t_Loc;
            tLog(m_pLogPBr, " %f ST #%d Passive path by descending branch", pInData->t_Loc, CurIndex);
        }
    ...
    {
        if (!pInData->bAeroballistic/*m_pInData_ST->Type != TYPE_QBM && m_pInData_ST->Type != TYPE_MaRV*/
                /*&& m_pInData_ST->TypeGT != TYPE_QBM && m_pInData_ST->TypeGT != TYPE_MaRV*/)
        {
            Path_Determination_ST(pInData);
        }
        else
        {       //for aeroballistic objects only
            ...
            {
                m_arPathBrData_ST[CurIndex].P_Path = AIR_BALL::PASSIVE_PATH;
                rOutData.P_Save_prev_PathBranchData = false;
                rOutData.P_Path = AIR_BALL::PASSIVE_PATH;
                tLog(m_pLogPBr, " %f ST #%d Passive path by aeroballistic", pInData->t_Loc, CurIndex);
            }
        }
    ...
    {
        rOutData.P_Save_prev_PathBranchData = true;
    }
    else //path or branch has changed
    {
        rOutData.P_Save_prev_PathBranchData = false;

        rOutData.P_Path = m_arPathBrData_ST[CurIndex].P_Path;
        rOutData.P_Branch = m_arPathBrData_ST[CurIndex].P_Branch;
    }

    tLog(m_pLogPBr, " %f ST #%d Path %d Branch %d", pInData->t_Loc, CurIndex,
         m_arPathBrData_ST[CurIndex].P_Path, m_arPathBrData_ST[CurIndex].P_Branch);
}


void CPathBranchDetermination_M::PathBranchDet_Realization_GT(const CPathBranch_InputData_GT* pInData,
                                                              CPathBranch_OutputData_GT& rOutData)
{
    ...
    for (it = pInData->SetST.begin(); it != pInData->SetST.end(); ++it)
    {
    ...
    {
        rOutData.bPathBranchChanged = true;
    ...
    {       //for aeroballistic objects only
        if (m_arPathBrData_GT[indGT].P_Path != AIR_BALL::PASSIVE_PATH)
        {
            ...
            tLog(m_pLogPBr, " %f GT #%d Passive path by aeroballistic", pInData->tLoc, indGT);
            rOutData.bPathBranchChanged = true;
        }
    }


    tLog(m_pLogPBr, " %f GT #%d Path %d Branch %d", pInData->tLoc, indGT,
         m_arPathBrData_GT[indGT].P_Path, m_arPathBrData_GT[indGT].P_Branch);
}

void CPathBranchDetermination_M::Path_Fusion_GT(const CPathBranch_InputData_GT* pInData,
                                                CPathBranch_OutputData_GT& rOutData)
{
...
//    const qreal EAP_FUSION_COEFFICIENT = 2;
    ...
    if (!allVoteForActive && !allVoteForPassive)
    {
        ...
        for (it = pInData->SetST.begin(); it != pInData->SetST.end(); it++)
        {
            ...
            if ((0 <= ind_ST && ind_ST < AIR_BALL::ST_FORM_AMOUNT)
                    && m_arPathBrData_ST[ind_ST].currentAhArraySize > 0
                    && m_p_Arr_BallProc->arData_ST[ind_ST].MeanPeriod > con_par_eps
                    && m_p_Arr_BallProc->arData_ST[ind_ST].tLoc > pInData->tPrevPathDet - meanSrcPeriod - con_eps
                    && m_p_Arr_BallProc->arData_ST[ind_ST].RMSE_mean < EAP_FUSION_COEFFICIENT * meanOverSoursesRMSE
                    && m_p_Arr_BallProc->arData_ST[ind_ST].MeanPeriod < EAP_FUSION_COEFFICIENT * meanSrcPeriod)
            {
                allVoteForPassive = allVoteForPassive && ((m_arPathBrData_ST[ind_ST].P_Path == AIR_BALL::PASSIVE_PATH) ||
    ...
    if ((allVoteForPassive) && (m_arPathBrData_GT[indGT].P_Path != AIR_BALL::PASSIVE_PATH))
    {
                ...
                (m_arPathBrData_GT[indGT].P_Path == AIR_BALL::UNDEFINED_PATH))
        {
            ...
            tLog(m_pLogPBr, " %f GT #%d End of active path", pInData->tLoc, indGT);
            rOutData.bPathBranchChanged = true;
            m_arPathBrData_GT[indGT].T_def_eap = pInData->tLoc;
            if (m_p_Arr_BallProc->arData_GT[indGT].Path == AIR_BALL::ACTIVE_PATH)
            {
...
void CPathBranchDetermination_M::Branch_Determination_ST(const CPathBranch_InputData_ST* pInData)
{
    const qint32 CurIndex = pInData->IF_ST;
    qint32 CurrBranch = AIR_BALL::UNDEFINED_BRANCH;
    Branch_Det_forCurrentUpd_ST(CurrBranch, pInData);
    m_arPathBrData_ST[CurIndex].P_Branch = CurrBranch;
}


void CPathBranchDetermination_M::Path_Determination_ST(const CPathBranch_InputData_ST* pInData)
{
     const qint32 CurIndex = pInData->IF_ST;
     qint32 CurrPath = AIR_BALL::UNDEFINED_PATH; //value of path for current update
     Path_Det_forCurrentUpd_ST(CurrPath, pInData);

     if (CurrPath == AIR_BALL::ACTIVE_PATH && pInData->pInDataST != nullptr)
     {
         GLCalcTrajParam TrajCalc;
         qreal ALong = TrajCalc.CalcLongitudinalAccel(pInData->pInDataST->Vel, pInData->pInDataST->Acc);
         qreal SigmaALong = TrajCalc.CalcRMSE_LongitudinalAccel(pInData->pInDataST->Vel, pInData->pInDataST->Acc,
                                                                    pInData->pInDataST->CovMatr);
         if (ALong > AIR_BALL::K_Sigma * SigmaALong)
         {
             ...
             m_arPathBrData_ST[CurIndex].T_act_prouved = pInData->t_Loc;
         }
     }
...
void CPathBranchDetermination_M::Branch_Det_forCurrentUpd_ST(qint32 &CurrBranch,
...
void CPathBranchDetermination_M::Path_Det_forCurrentUpd_ST(qint32 &CurrPath,
                                                           const CPathBranch_InputData_ST *pInData)
{
    const qint32 CurIndex = pInData->IF_ST;

    qint16 Path_VH = EstimatePathByVertVel(pInData); //preliminary estimated path, using vertical velocity

    if( m_arPathBrData_ST[CurIndex].currentFilterArraySize >= MIN_OVERWEIGHT_POINT_NUMBER)
    {
        const qreal sigma_h_for_filter = sqrt(pow(SIGMA_RANGE,2.) + pow(pInData->R_rdr_obj,2.) * pow(SIGMA_EPSILON,2.));
        ...
        if(sigma_h_for_filter < MAX_ALLOWABLE_H_RMSE_FOR_FILTER)
        {
            ...
            if(flag_filter)
            {
                ...
                tLog(m_pLogPBr, " %f ST #%d Passive path by the filter", pInData->t_Loc, CurIndex);
                return;
            }
        }
    }
    ...
    const qreal g_h = con_g * pow(con_Eath_middle_radius / ( con_Eath_middle_radius + pInData->TrParameters.H), 2.);
    ...
    if (Ah < Ah_FOR_PASSIVE_PATH_ACCEPT
            && pInData->TrParameters.H < H_ATMOSPHERE)
    {
        CurrPath = AIR_BALL::PASSIVE_PATH;
        tLog(m_pLogPBr, " %f ST #%d Passive path by the threshold", pInData->t_Loc, CurIndex);
        return;
    }
    ...
    if( criterion_Value > BOUND_AH_EAP_ACCEPT
            || pInData->TrParameters.a > g_h + cCoef_a_g_sigA * AIR_BALL::K_Sigma * pInData->TrParameters.SigA)
    {
        ...
        if (fabs(m_arPathBrData_ST[CurIndex].T_def_eap - AIR_BALL::Timer_INI) > con_eps
                && pInData->t_Loc - m_arPathBrData_ST[CurIndex].T_def_eap > DT_PATH_REDETERMINATION_CONTROL)
        {
            ...
            {
                m_arPathBrData_ST[CurIndex].t0_active_redetermination = pInData->t_Loc;
            }
            else
            {
                if (pInData->t_Loc - m_arPathBrData_ST[CurIndex].t0_active_redetermination > DT_CTRL_PATH_REDETERM
    ...
    else
    {
        ...
        {
            CurrPath = AIR_BALL::PASSIVE_PATH;
            tLog(m_pLogPBr, " %f ST #%d Passive path by the criterion", pInData->t_Loc, CurIndex);
...
qint16 CPathBranchDetermination_M::EstimatePathByVertVel(const CPathBranch_InputData_ST *pInData)
{
...
void CPathBranchDetermination_M::Branch_Check_ST(const CPathBranch_InputData_ST* pInData)
{
    const qint32 CurIndex = pInData->IF_ST;
    qint32 CurrBranch = AIR_BALL::UNDEFINED_BRANCH;
    Branch_Det_forCurrentUpd_ST(CurrBranch, pInData);
          ...
          && (CurrBranch != m_arPathBrData_ST[CurIndex].P_Branch))
    {
        if (CurrBranch == m_arPathBrData_ST[CurIndex].Branch_saved_for_check)
        {
            ...
            {
                m_arPathBrData_ST[CurIndex].t_begin_check_branch = pInData->t_Loc;
            }

            m_arPathBrData_ST[CurIndex].counter_check_branch ++;

            if (pInData->t_Loc - m_arPathBrData_ST[CurIndex].t_begin_check_branch > PERIOD_BRANCH_CHECK_CONFIRM
...
//void CPathBranchDetermination_M::EAP_Det_VDoppl_ST()
//{
//    ;
//}


void CPathBranchDetermination_M::VhArrayFilling(const CPathBranch_InputData_ST* pInData)
{
    const qint32 CurIndex = pInData->IF_ST;
    ...
    m_arPathBrData_ST[CurIndex].arr_TVh[Curr_Vh_Ind] = pInData->t_Loc;
    m_arPathBrData_ST[CurIndex].arr_Vh[Curr_Vh_Ind] = pInData->TrParameters.Vh;
    m_arPathBrData_ST[CurIndex].arr_SigVh[Curr_Vh_Ind] = pInData->TrParameters.SigVh;

    m_arPathBrData_ST[CurIndex].arr_Ah[Curr_Ah_Ind] = pInData->TrParameters.ah_noC; //ah;
    m_arPathBrData_ST[CurIndex].arr_SigAh[Curr_Ah_Ind] = pInData->TrParameters.SigAh + CORR_RMSE_AH;

    m_arPathBrData_ST[CurIndex].arr_Prob_IMM_small_coeff[Curr_filter_Ind] = pInData->Prob_IMM_small_coeff;
    m_arPathBrData_ST[CurIndex].arr_Prob_IMM_large_coeff[Curr_filter_Ind] = pInData->Prob_IMM_large_coeff;
...
void CPathBranchDetermination_M::DepthFindingInVhCalculating()
{
    ;
}


void CPathBranchDetermination_M::ClarifyEAP_Time()
{
    ;
}


void CPathBranchDetermination_M::Drop_GT(const qint32 ind_GT)
{
...
void CPathBranchDetermination_M::Drop_ST(const qint32 ind_ST)
{
    if (0 <= ind_ST && ind_ST < AIR_BALL::ST_FORM_AMOUNT)
    {
...
void CPathBranchDetermination_M::ClearInnerData_GT()
{
...
void CPathBranchDetermination_M::ClearInnerData_ST()
{
...
void CPathBranchDetermination_M::ClearInnerData_GT_1Track(const qint32 index)
{
...
void CPathBranchDetermination_M::ClearInnerData_ST_1Track(const qint32 index)
{
